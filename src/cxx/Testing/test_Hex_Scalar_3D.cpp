...
TEST_CASE("Test analytic eigenfunction solution for scalar "
  ...
  char order_str[5];
  for (int poly_order = 2; poly_order < 6; poly_order++) {
    ...
    const char *arg[] = {
      ...
      "--time-step", "1e-2",
      "--polynomial-order", order_str, NULL};
    ...
    while (true) {
      ...
      problem->saveSolution(time, {"a","u"}, fields, mesh->DistributedMesh());
      max_error = element_error.maxCoeff() > max_error ? element_error.maxCoeff() : max_error;
      if (time > cycle_time) break;

    }
    
    DEBUG() << "hex order " << options->PolynomialOrder() << " error: " << max_error;
