...
bool QuadP1::checkHull(const PetscReal x, const PetscReal y, const Ref<const QuadVtx>& vtx) {
  ...
  PetscReal detJac;
  RealMat2x2 invJac;
  std::tie(invJac, detJac) = inverseJacobianAtPoint(0, 0, vtx, detJac, invJac);
  if (detJac <= 0) {
...
std::tuple<RealMat2x2, PetscReal> QuadP1::inverseJacobianAtPoint(
  ...
  PetscReal r_ = (r + 1.0);
  PetscReal s_ = (s + 1.0);
  PetscReal detJ = -r_ * v1x * v3y / 8 + r_ * v1x * v4y / 8 + r_ * v1y * v3x / 8 - r_ * v1y *
  ...
  PetscReal rx = (1 / detJ) * (r_ * (v1y - v2y) / 4 + r_ * (v3y - v4y) / 4 - v1y / 2 + v4y / 2);
  PetscReal rz = (1 / detJ) * (-r_ * (v1x - v2x) / 4 - r_ * (v3x - v4x) / 4 + v1x / 2 - v4x / 2);
  PetscReal sx = (1 / detJ) * (-s_ * (v1y - v2y + v3y - v4y) / 4 + v1y / 2 - v2y / 2);
  PetscReal sz = (1 / detJ) * (s_ * (v1x - v2x + v3x - v4x) / 4 - v1x / 2 + v2x / 2);

  RealMat2x2 inverseJacobian;
  inverseJacobian <<  rx, sx, rz, sz;
